        ...
        for member in members_added:
            if member.id != turn_context.activity.recipient.id:
                await turn_context.send_activity(
                    "Hello and welcome! Please type 'login' for initiating the authentication flow."
                )

    # Handles token response events by continuing the dialog.
    async def on_token_response_event(self, turn_context: TurnContext):
        logging.info("Running dialog with Token Response Event Activity.")

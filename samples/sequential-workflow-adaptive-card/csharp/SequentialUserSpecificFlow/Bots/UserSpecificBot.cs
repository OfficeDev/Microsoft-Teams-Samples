...
namespace SequentialUserSpecificFlow.Bots
{
    public class UserSpecificBot : ActivityHandler
    {
        ...
        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            var initialAdaptiveCard =  firstOptionsAdaptiveCard(path,turnContext.Activity.From.Name,member.Id);
            await turnContext.SendActivityAsync(MessageFactory.Attachment(initialAdaptiveCard), cancellationToken);
        }

        private  Attachment firstOptionsAdaptiveCard(string[] filepath, string name=null, string userMRI=null)
        {
            ...
            if (turnContext.Activity.Name == "adaptiveCard/action")
            {
                ...
                switch (verb)
                {
                    case "initialRefresh":
                        ...
                        adaptiveCardResponse = createCard(initialCard, data);
                    ...
                    case "firstCard":
                        ...
                        adaptiveCardResponse = createCard(firstCard, data);
                    ...
                    case "secondCard":
                        ...
                        {
                            adaptiveCardResponse = createCard(secondCard, data, Constants.Software);
                        }
                        else
                        {
                            adaptiveCardResponse = createCard(secondCard, data, Constants.Hardware);
                        }
                    ...
                    case "thirdCard":
                        ...
                        var responseAttachment = reviewCard(thirdCard, data,out cardJson);
                    ...
                    case "refresh":
                        ...
                        adaptiveCardResponse = createCard(assignedToCard, data);
                        return CreateInvokeResponse(adaptiveCardResponse);

                    case "approved":

                        string[] approvedCard = { ".", "Resources", "approvedCard.json" };
                        var approvedAttachment = reviewCard(approvedCard, data, out cardJson);
                    ...
                    case "rejected":
                        ...
                        var rejectedAttachment = reviewCard(rejectedCard, data, out cardJson);
        ...
        public AdaptiveCardInvokeResponse createCard(string[] path,InitialSequentialCard data, List<string> subCategory= null)
        {
        ...
        private Attachment reviewCard(string[] filepath, InitialSequentialCard data, out string cardJsonString)
        {
            var adaptiveCardJson = File.ReadAllText(Path.Combine(filepath));
            AdaptiveCardTemplate template = new AdaptiveCardTemplate(adaptiveCardJson);
            var payloadData = new

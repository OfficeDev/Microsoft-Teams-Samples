...
1) Update the `.localConfigs` configuration for the bot to use the Microsoft App Id and App Password from the Bot Framework registration. (Note the App Password is referred to as the "client secret" in the azure portal and you can always create a new client secret anytime.) `CLIENT_ID` will be the id for the tenant where application is registered.
   - Set "MicrosoftAppType" in the `localConfigs`. (**Allowed values are: MultiTenant(default), SingleTenant, UserAssignedMSI**)
...
<img src="https://pnptelemetry.azurewebsites.net/microsoft-teams-samples/samples/msgext-action-nodejs" />
...
async def handle_message_ext_query(ctx: ActivityContext[MessageExtensionQueryInvokeActivity]):
    ...
    if command_id == "searchQuery":
        ...
        for item in results:
            # Create Adaptive Card for main content using model_validate
            adaptive_card = AdaptiveCardAttachment(
                content=AdaptiveCard.model_validate({
                    "type": "AdaptiveCard",
                    "version": "1.4",
                    "body": [
                        {
                        ...
                        {
                            "type": "TextBlock",
                            "text": item["summary"][:100] if item["summary"] else "No description",
                            "wrap": True
                        }
                    ],
                    "actions": [

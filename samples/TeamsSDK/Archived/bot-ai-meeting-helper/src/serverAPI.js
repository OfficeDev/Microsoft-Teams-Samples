...
const credentials = new MicrosoftAppCredentials(config.BOT_ID, config.BOT_PASSWORD);
...
async function transcribeAndExtractUserActionItems(meetingNotes, userinfo, meetingDetails) {
  ...
  const deployment_Id = config.AZURE_OPENAI_DEPLOYMENT_NAME;
  ...
  try {
       ...
       client.trackEvent({ name: "subscription", properties: { transcribeAndExtractUserActionItems: meetingNotes } });
        ...
        const fileChunks = [];
        for (let i = 0; i < meetingNotes.length; i += 3000) {
          fileChunks.push({ pageContent: meetingNotes.slice(i, i + 3000) });
        }

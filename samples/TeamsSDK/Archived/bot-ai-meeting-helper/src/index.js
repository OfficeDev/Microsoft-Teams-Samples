...
server.post("/EventHandler", async (req, res) => {
  ...
  {
    const notification = req.body.value;
      ...
      { 
        ...
        .then(async meetingDetails => {
            ...
            .then(async onlineMeetingDetail => {
              ...
              GetUserInformationAndSendActionItems(req.body.value[0].resource, meetingDetails, onlineMeetingDetail, req.body.value[0].subscriptionId);
...
server.post("/api/messages", async (req, res) => { 
  // Route received a request to adapter for processing
    await adapter.process(req, res, async (context) => {
      ...
      {
        // No-op for non-message/invoke activities.
      }      
    });
});
...
{  
  try
  {
      ...
      let UserInformation = "";
        ...
        await tableStorageService.storeData(config.partitionKey, crypto.randomUUID(), data);
...
async function GetUserInformationAndSendActionItems(onlineMeetingId, AIPrompt, onlineMeetingDetail, subscriptionId)
{
  try
  {
    ...
    .then(async UserAndMeeting => {
        ...
        await serverAPI.transcribeAndExtractUserActionItems(AIPrompt, UserAndMeeting, onlineMeetingDetail);
    })
  .catch(error => {

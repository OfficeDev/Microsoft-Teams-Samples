...
**Latest Updates**: This sample has been modernized with .NET 10, cleaned-up code, removed dead code, and configured as a single-tenant application for simpler deployment.
...
## Set-up Overview
The following instructions will walk developers through the steps required to setup the application as a single-tenant Teams app.


### Prerequisites
- Admin user credentials on 1 M365 Tenant
- [Git](https://github.com/git-guides/install-git)
- [Ngrok](https://ngrok.com/download) or [Dev Tunnels](https://learn.microsoft.com/en-us/aspnet/core/test/dev-tunnels)
- [Microsoft Teams](https://www.microsoft.com/en-ca/microsoft-teams/log-in)
- [Visual Studio 2022](https://visualstudio.microsoft.com/) or later with .NET 10 support
...
2. Select **New Registration** and on the *register an application page*, set following values:
    ...
    * Choose **Accounts in this organizational directory only** to keep the sample single-tenant.
...
3. Complete the [setup](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/tab-request-approval/csharp#setup) for the original Tab Request Approval app in your tenant for **this** repo.
4. Complete reading the [SharePoint Embedded documentation](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/overview) section for SharePoint Embedded in your tenant.
5. Configuring app secrets

    - Values to obtain from your tenant's Azure portal.
        - ```$TenantId``` represents your tenant id.
    ...
    - Values to obtain from the SharePoint Embedded setup procedure.
        ...
        - Navigate to the **Certificates and Secrets** tab in your Azure portal and click on the **Certificates** section and upload the ```.cer```.
...
6. Deploy the Teams App
    - Upload the ```manifest.zip``` of the teams app into the [Teams Admin Center](https://admin.teams.microsoft.com/policies/manage-apps) by clicking on the **Upload new app** button
    - Once uploaded, grant permissions to the app by clicking on the **Permissions** tab, then clicking on the **Grant Admin Consent** button or **Review permissions** button.
...
1. I'm running into errors involving granting admin consent when I am uploading the manifest in the Teams Admin Center.
    
    - Undo/ delete the changes on Azure AD (Redirect URIs, API Permissions, Exposed an API, Secrets, Certificates) that integrating SharePoint Embedded specifically made. Then upload the manifest with only the Tab Request Approval information and grant admin consent. Then integrate SharePoint Embedded, update the manifest and retry the upload process.

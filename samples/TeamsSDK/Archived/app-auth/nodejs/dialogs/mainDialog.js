...
class MainDialog extends ComponentDialog {
    ...
    */
    ...
    async promptStep(stepContext) {
        try {
            const userInput = stepContext.context._activity.text.trim().toLowerCase();
            
            if (userInput === "sso") {
                return await stepContext.beginDialog(SSOAUTH);
            } else if (userInput === "facebooklogin") {
            ...
            console.error('Error in promptStep:', err);
            await stepContext.context.sendActivity('An error occurred. Please try again.');
            return await stepContext.endDialog();
        }
    }
}

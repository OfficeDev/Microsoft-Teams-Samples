...
4. Navigate to **API Permissions**, and make sure to add the following permissions:
    ...
    * Click on **Add permissions** and grant admin consent for the required permissions.

   > Note: This sample does not call Microsoft Graph at runtime — `User.Read` is the default delegated permission for any newly registered Microsoft Entra ID app and is sufficient for the messaging extension to function.
...
5) In a terminal, go to `samples\msgext-action-quickstart`
...
<img src="https://pnptelemetry.azurewebsites.net/microsoft-teams-samples/samples/TeamsSDK/Archived/msgext-action-quickstart-python" />

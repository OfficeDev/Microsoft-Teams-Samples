    ...
    constructor() {
        ...
        this.onMessage(async (context, next) => {
            ...
            if (adaptiveFormatCards[text]) {
                ...
                if ('rating1' in activityValue && 'rating2' in activityValue) {
    ...
    async sendWelcomeMessage(turnContext) {
        ...
        for (const member of activity.membersAdded) {
            if (member.id !== activity.recipient.id) {
                await turnContext.sendActivity('Welcome to Adaptive Card Format. This bot will introduce you to different types of formats. Please select the cards from given options');

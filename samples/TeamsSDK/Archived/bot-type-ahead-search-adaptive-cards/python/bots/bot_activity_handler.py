...
class TeamsBot(ActivityHandler):
        ...
        for member in members_added:
            if member.id != context.activity.recipient.id:
                await context.send_activity(
                    "Hello and welcome! With this sample you can see the functionality of "
                    "static and dynamic search in adaptive card"
                )
    ...
    async def on_message_activity(self, context: TurnContext):
        ...
        if text == "staticsearch":
            card = CardFactory.adaptive_card(self._adaptive_card_for_static_search())
            await context.send_activity(Activity(type=ActivityTypes.message, attachments=[card]))
        elif text == "dynamicsearch":
            card = CardFactory.adaptive_card(self._adaptive_card_for_dynamic_search())
            await context.send_activity(Activity(type=ActivityTypes.message, attachments=[card]))
        elif text == "dependantdropdown":
            card = CardFactory.adaptive_card(self._adaptive_card_for_dependant_search())
            await context.send_activity(Activity(type=ActivityTypes.message, attachments=[card]))
    ...
    def _build_search_response(self, dropdown_card, npm_packages):
        ...
        results = (
            country_options.get(dropdown_card.lower(), [])
            if dropdown_card
            else npm_packages
        )
    ...
    @staticmethod
    def _adaptive_card_for_static_search():
        return {
    ...
    @staticmethod
    def _adaptive_card_for_dynamic_search():
        return {
            "$schema": "http://adaptivecards.io/schemas/adaptive-card.json",
            "version": "1.6",
            "type": "AdaptiveCard",
    ...
    @staticmethod
    def _adaptive_card_for_dependant_search():
        return {
            "type": "AdaptiveCard",

...
const { SsoOAuthHelper } = require('../ssoOauthHelper');
...
class TeamsBot extends DialogBot {
    ...
    */
    constructor(conversationState, userState, dialog) {
        ...
        this._ssoOAuthHelper = new SsoOAuthHelper(process.env.connectionName, conversationState);

        this.onMembersAdded(async (context, next) => {
            const membersAdded = context.activity.membersAdded;
            for (let member = 0; member < membersAdded.length; member++) {
                if (membersAdded[member].id !== context.activity.recipient.id) {
                    await context.sendActivity('Hello and welcome! This sample demonstrates how to fetch shared images and files in Microsoft Teams group chats and team channels using the Graph API. Please add this bot to a team or group chat and share a file.');
                }
            }

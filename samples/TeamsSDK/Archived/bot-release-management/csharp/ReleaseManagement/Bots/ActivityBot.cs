...
namespace ReleaseManagement.Bots
{
    ...
    public class ActivityBot : ActivityHandler
    {
        ...
        {
            var isPresent = taskDetails.TryGetValue(Constant.TaskDetails, out var releaseManagementTask);

            // Checking if task details is present and if bot is installed.
            if (isPresent && turnContext.Activity.MembersAdded.Count > 0)
            {
                var adaptiveAttachment = cardFactory.CreateAdaptiveCardAttachement(
                    Path.Combine(".", "Resources", "WorkitemCardTemplate.json"),

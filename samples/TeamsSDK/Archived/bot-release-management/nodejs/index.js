...
global.taskDetails = Object.create(null);
...
server.post('/api/workItem', async (req, res) => {
    ...
    const releaseManagementTask = await DevOpsHelper.MapToReleaseManagementTask(req.body);

    global.taskDetails[Constant.TaskDetails] = releaseManagementTask;

    if (releaseManagementTask.GroupChatMembers.length > 1)
    {
        const graphHelper = new GraphHelper();
        const groupChat = await graphHelper.CreateGroupChatAsync(releaseManagementTask.GroupChatMembers, releaseManagementTask.TaskTitle);

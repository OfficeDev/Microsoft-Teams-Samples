...
namespace CallingBotSample.Authentication
{
    public class AuthenticationProvider : ObjectRoot, IRequestAuthenticationProvider
    {
        ...
        public async Task AuthenticateOutboundRequestAsync(HttpRequestMessage request, string tenant)
        {
            ...
            var credential = new ClientSecretCredential(tenant, this.appId, this.appSecret);
            Azure.Core.AccessToken tokenResult;
            try
            {
                tokenResult = await this.AcquireTokenWithRetryAsync(credential, resource, attempts: 3).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
            ...
            this.GraphLogger.Info($"AuthenticationProvider: Generated OAuth token. Expires at {tokenResult.ExpiresOn}.");

            request.Headers.Authorization = new AuthenticationHeaderValue(schema, tokenResult.Token);
        }
        ...
        /// </summary>
        /// <param name="credential">The client secret credential.</param>
        /// <param name="scope">The scope.</param>
        /// <param name="attempts">The attempts.</param>
        /// <returns>
        /// The <see cref="Azure.Core.AccessToken" />.
        /// </returns>
        private async Task<Azure.Core.AccessToken> AcquireTokenWithRetryAsync(ClientSecretCredential credential, string scope, int attempts)
        {
            while (true)
            {
                attempts--;
                ...
                try
                {
                    return await credential.GetTokenAsync(new Azure.Core.TokenRequestContext(new[] { scope }), CancellationToken.None).ConfigureAwait(false);
                }
                catch (Exception)
                {
                    if (attempts < 1)

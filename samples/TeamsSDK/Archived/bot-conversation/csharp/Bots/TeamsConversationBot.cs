...
public class TeamsConversationBot : TeamsActivityHandler
{
    ...
    private static ConcurrentDictionary<string, TeamsChannelAccount> _teamMemberDetails = new();
    private static ConcurrentDictionary<string, string> _teamMemberMessageIdDetails = new();
    ...
    public TeamsConversationBot(IConfiguration config)
    {
        ...
        _appPassword = config["MicrosoftAppPassword"] ?? string.Empty;
    }
        ...
        /// <param name="cancellationToken"></param>
        /// <returns>A task that represents the work queued to execute.</returns>
        private Task ResetReadUserCount(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            _teamMemberDetails.Clear();
            _counter = 0;
            _users.Clear();
            _teamMemberMessageIdDetails.Clear();
        ...
        {
            if (_teamMemberMessageIdDetails.TryGetValue(turnContext.Activity.From.AadObjectId, out var messageId)
                && readReceiptInfo.IsMessageRead(messageId))
            {
                if (_teamMemberDetails.TryGetValue(turnContext.Activity.From.AadObjectId, out var memberDetails))
                {
                    _counter++;
                    _users.Add(memberDetails.Name);
                }

                _teamMemberMessageIdDetails.TryRemove(turnContext.Activity.From.AadObjectId, out _);
            }
        ...
        private async Task GetSingleMemberAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            await turnContext.SendActivityAsync(MessageFactory.Text($"You are: {member.Name}."), cancellationToken);
        }
        ...
        private async Task MessageAllMembersAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken, bool isAadId)
        {
            ...
            foreach (var teamMember in members)
            {
                ...
                var conversationParameters = new ConversationParameters
                {
                    ...
                    Members = isAadId ? [new ChannelAccount(teamMember.AadObjectId)] : [teamMember],

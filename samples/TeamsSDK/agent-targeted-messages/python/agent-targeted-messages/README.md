...
The agent uses the Teams SDK (`microsoft-teams-apps` / `microsoft-teams-api`) to send targeted messages:
...
- **`strip_mentions_text(activity, StripMentionsTextOptions(account_id=...))`** — strips agent or user @mentions from incoming message text.
...
- [Send and receive messages with an agent](https://learn.microsoft.com/en-us/microsoftteams/platform/bots/how-to/conversations/conversation-messages)

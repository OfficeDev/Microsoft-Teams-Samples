...
const ViewEditTag = props => {
    ...
    const onTopicNameUpdate = (updatedText) => {
        setEditedTopicName(updatedText.trim());
        setIsNoteDisabled(false);
    }

    // Handler when description is updated.
    const onstartDateUpdate = (updatedText) => {
        setEditedStartDate(updatedText.trim());
        setIsNoteDisabled(false);
    }
    // Handler when description is updated.
    const onEndDateUpdate = (updatedText) => {
        setEditedendDate(updatedText.trim());
        setIsNoteDisabled(false);
        
    }
    const onattendeesUpdate = (updatedText) => {
        setEditedattendees(updatedText.trim());
        setIsNoteDisabled(false);        
    }
    ...
    const onUpdateButtonClick = async () => {
        ...
        microsoftTeams.app.getContext().then(async (context) => {
            var updateTagDto = {
                ...
                topicName: editedTopicName === "" ? props.teamworkTag.topicName : editedTopicName,                
                //start: editedStartDate === "" ? props.teamworkTag.start : editedStartDate,
                //end: editedEndDate === "" ? props.teamworkTag.start : editedEndDate,
                //attendees: editedattendees === "" ? props.teamworkTag.attendees : editedattendees
            };    

            var response = await axios.patch(`api/eventupdate/${teamId}`, updateTagDto);
    ...
    const renderTagInformation = () => {
        return (<Flex column gap="gap.medium">
            <Flex gap="gap.small" vAlign="center">
                <Text className={props.dashboardState === DashboardState.View ? "text-fields-label" : "input-fields-label"} content={`Training Subject:`} weight="semibold" />
                {
                    props.dashboardState === DashboardState.View ?
                        <Text content={props.teamworkTag.topicName} /> :
                        <Input fluid defaultValue={props.teamworkTag.topicName} onChange={(event, data) => { onTopicNameUpdate(data.value) }} />
                }
            </Flex>

            <Flex gap="gap.small" vAlign="center">
                <Text className={props.dashboardState === DashboardState.View ? "text-fields-label" : "input-fields-label"} content={`Start Date:`} weight="semibold" />
                {
                    props.dashboardState === DashboardState.View ?
                        <Text content={JSON.stringify(props.teamworkTag.start)} /> :
                        <TextArea fluid className="input-fields" defaultValue={JSON.stringify(props.teamworkTag.start)} onChange={(event, data) => { onstartDateUpdate(data.value) }} />
                }
            </Flex>
            <Flex gap="gap.small" vAlign="center">
                <Text className={props.dashboardState === DashboardState.View ? "text-fields-label" : "input-fields-label"} content={`End Date:`} weight="semibold" />
                {
                    props.dashboardState === DashboardState.View ?
                        <Text content={JSON.stringify(props.teamworkTag.end)} /> :
                        <TextArea fluid className="input-fields" defaultValue={JSON.stringify(props.teamworkTag.end)} onChange={(event, data) => { onEndDateUpdate(data.value) }} />
                }
            </Flex>

            <Flex gap="gap.small" vAlign="center">
                <Text content={`Meeting Participants:`} weight="semibold" />
                <Text content={JSON.stringify(props.teamworkTag.attendees)} />
            </Flex>
        </Flex>);
    }

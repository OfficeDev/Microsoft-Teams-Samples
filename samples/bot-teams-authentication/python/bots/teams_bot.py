

from typing import List
from botbuilder.core import (
    ConversationState,
    UserState,
    TurnContext,
)
from botbuilder.dialogs import Dialog
from botbuilder.schema import ChannelAccount

from helpers.dialog_helper import DialogHelper
from botbuilder.schema import InvokeResponse
from bots.dialog_bot import DialogBot
import logging

class TeamsBot(DialogBot):
    def __init__(
        self,
        conversation_state: ConversationState,
        user_state: UserState,
        dialog: Dialog,
    ):
        super(TeamsBot, self).__init__(conversation_state, user_state, dialog)

    async def on_members_added_activity(
        self, members_added: List[ChannelAccount], turn_context: TurnContext
    ):
        for member in members_added:
            if member.id != turn_context.activity.recipient.id:
                await turn_context.send_activity(
                    "Welcome to AuthenticationBot.\n\n"
                    "Type anything to log in.\nType 'logout' to sign out."
                )

    async def on_teams_signin_verify_state(self, turn_context: TurnContext):
        print("Running dialog with signin/verifyState from an Invoke Activity.")
        await DialogHelper.run_dialog(
            self.dialog,
            turn_context,
            self.conversation_state.create_property("DialogState"),
        )
        return InvokeResponse(status=200)


    async def on_teams_signin_token_exchange(self, turn_context: TurnContext):
        print("Running dialog with signin/tokenExchange from an Invoke Activity.")
        await DialogHelper.run_dialog(
            self.dialog,
            turn_context,
            self.conversation_state.create_property("DialogState"),
        )
        return InvokeResponse(status=200)
        
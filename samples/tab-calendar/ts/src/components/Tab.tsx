...
class Tab extends React.Component<ITabProps, ITabState> {
  ...
  composeMeeting() {
    ...
    if ((this.state.subject !== undefined && this.state.subject !== null && this.state.subject !== "") &&
      (this.state.content !== undefined && this.state.content !== null && this.state.content !== "") &&
      (this.state.attendees !== undefined && this.state.attendees !== null && this.state.attendees !== "") &&
      (this.state.stateDatetime !== undefined && this.state.stateDatetime !== null && this.state.stateDatetime !== "") &&
      (this.state.endDatetime !== undefined && this.state.endDatetime !== null && this.state.endDatetime !== "")) {
      isValidation = true;
    }
  ...
  render() {
        ...
        <form name="ComposeMeetingForm">
          <table>
            <tr>
              ...
              <td> <input type="text" className='txtValue' placeholder="Attendees" required value={(this.state.attendees || '').toString()} onChange={this.attendeesHandleChange.bind(this)} /></td>
            </tr>
            <tr>
              <td><label className='lblText'>Start Time:</label></td>
              <td><input type="datetime-local" className='dateTimeValue' required value={(this.state.stateDatetime || '').toString().substring(0, 16)} onChange={this.startHandleChange.bind(this)} /></td>
            </tr>
            <tr>
              <td><label className='lblText'>End Time:</label></td>
              <td><input type="datetime-local" className='dateTimeValue' required value={(this.state.endDatetime || '').toString().substring(0, 16)} onChange={this.endHandleChange.bind(this)} /></td>
            </tr>
            <tr>
              <td> <label className='lblText'>Subject:</label></td>
              <td> <input type="text" className='txtValue' placeholder="Subject" value={(this.state.subject || '').toString()} onChange={this.subjectHandleChange.bind(this)} required /></td>
            </tr>
            <tr>
              <td> <label className='lblText'>Content:</label></td>
              <td><textarea required  className='dateTimeValue' placeholder="Type your meeting content..." rows={4} cols={40} value={(this.state.content || '').toString()} onChange={this.contentHandleChange.bind(this)} /></td>
            </tr>
            <tr>
              <td></td>
              <td><button onClick={this.composeMeeting.bind(this)}>Compose Meeting!</button></td>
            </tr>
          </table>
        </form>
      </div>
      ...
      { 
      this.state.allItems.slice(0, 3).map((value: IAllItemsDetail, index) => {
            return <tr>
          <td className='tdSubject'>{value.subject.substring(0, 30)}</td>
          <td><button onClick={() => this.openCalendar(value.id)}>Calendar!</button></td>
        </tr>
      })
     }
      </table>

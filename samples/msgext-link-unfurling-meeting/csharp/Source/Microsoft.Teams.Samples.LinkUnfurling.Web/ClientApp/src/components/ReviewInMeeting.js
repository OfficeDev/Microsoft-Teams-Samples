...
export class ReviewInMeeting extends Component {
    ...
    initiateSSO() {
        // Get SSO token.
        microsoftTeams.authentication.getAuthToken().then((ssoToken) => {
    ...
    reviewDashboard = async () => {
        ...
        let conversationId = this.state.context?.chat?.id;
        if (this.state.context?.team?.internalId) {
            conversationType = "channel";
            conversationId = this.state.context.channel.id;
            teamId = this.state.context.team.internalId;
        }

...
app.ai.action('EchoBot', async (context, state) => {
    try {
        // context.sendActivity("Echoing: " + context.activity.text);
...
app.ai.action('TranslationBot', async (context, state) => {
    try {
        // context.sendActivity("Translating: " + context.activity.text);
        // Send the activity to the skill
...
async function sendToSkill(context, targetSkill) {
    try {
        ...
        // Log request details
        console.log("Sending activity to skill with ID:", targetSkill.id);
        console.log("Skill endpoint:", targetSkill.skillEndpoint);
        console.log("Skill conversation ID:", skillConversationId);
        ...
        console.log("Skill response body:", response.body);

        // Track the event after sending the message
        appInsightsClient.trackEvent({ name: 'Skill response status:', properties: { message: response.status } });
        // Track the event after sending the message
        appInsightsClient.trackEvent({ name: 'Skill response body:', properties: { message: response.body } });

        // Check response status
        if (!(response.status >= 200 && response.status <= 299)) {
            // Track the exception with Application Insights
            appInsightsClient.trackException({ exception: response });
        ...
        await context.sendActivity(`Error in sendToSkill: ${targetSkill.skillEndpoint} Error: ${err}`);
        console.log("Error in sendToSkill:", err);

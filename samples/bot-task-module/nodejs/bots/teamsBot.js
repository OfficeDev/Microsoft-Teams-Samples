...
class TeamsBot extends TeamsActivityHandler {
    constructor() {
        ...
        this.onMessage(async (context, next) => {
            ...
            else if (context.activity.value.Type == "SendInStageNotification") {
                var adaptiveCardChoiceSet = context.activity.value.Choice;
                var selectedMembers = adaptiveCardChoiceSet.split(",");
                // var selectedMembers = adaptiveCardChoiceSet;
                this.targetedNotification_InStage(context, meetingId, selectedMembers);
            }
            else if (context.activity.value.Type == "TabNotification") {
                var adaptiveCardChoiceSet = context.activity.value.Choice;
                var selectedMembers = adaptiveCardChoiceSet.split(",");
                // var selectedMembers = adaptiveCardChoiceSet;
                this.targetedNotification_InTab(context, meetingId, selectedMembers);
            }
            else {
    ...
    async targetedNotification_InStage(context, meetingId, selectedMembers) {
        ...
        try {
            //console.log("Data Test--->",JSON.stringify(notificationInformation.value.surfaces));
    ...
    // Custom method for sending Visual Indicator In Tab.
    async targetedNotification_InTab(context, meetingId, selectedMembers) {
        // Notification payload for meeting target notification API.
        let notificationInformation = {
            type: "targetedMeetingNotification",
            value: {
